...
const InfoPanel = ({ chat, onClose }: InfoPanelProps) => {
  ...
  const status = isGroup ? `${chat.members.length + 1} participants` : member?.is_online ? 'Online' : 'Offline';
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col h-full bg-card">
      <div className="bg-primary px-4 py-3 flex items-center justify-between">
        <h3 className="font-semibold text-sm text-primary-foreground">{isGroup ? 'Group Info' : 'Contact Info'}</h3>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex flex-col items-center py-6 px-4">
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold overflow-hidden text-primary-foreground"
            style={{ background: (isGroup ? chat.group_avatar_url : member?.avatar_url) ? undefined : `hsl(${hue}, 60%, 55%)` }}>
          ...
          <h2 className="font-bold text-base mt-3 text-foreground">{name}</h2>
          <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5">
            {!isGroup && member?.is_online && <span className="w-2 h-2 rounded-full status-online inline-block" />}
            {status}
          </p>

          <div className="flex gap-6 mt-5">
            {[{ icon: Phone, label: 'Audio' }, { icon: Video, label: 'Video' }, { icon: Bell, label: 'Mute' }].map(action => (
              <button key={action.label} className="flex flex-col items-center gap-1.5 group">
                <div className="p-2.5 rounded-full bg-primary/10 group-hover:bg-primary/20 text-primary transition-all">
                  <action.icon size={17} />
                </div>
        ...
        {!isGroup && member && (
          <div className="px-4 pb-4">
            <div className="bg-muted/50 rounded-xl p-3">
              ...
              <p className="text-sm text-foreground">{member.about || 'Hey there! I am using HyperChat'}</p>
            </div>
          </div>
        )}
        ...
        <div className="px-4 space-y-0.5">
          {sections.map(s => (
            <button key={s.label} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors">
              <s.icon size={15} className="text-muted-foreground" />
              <span className="text-sm flex-1 text-left text-foreground">{s.label}</span>
        ...
        {isGroup && (
          <div className="px-4 mt-4 pb-4">
            ...
            {chat.members.map(m => (
              <div key={m.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-muted/50 transition-colors">
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold overflow-hidden flex-shrink-0 text-primary-foreground"
                  style={{ background: m.avatar_url ? undefined : `hsl(${avatarHue(m.display_name)}, 60%, 55%)` }}>
                  {m.avatar_url ? <img src={m.avatar_url} alt="" className="w-full h-full object-cover" /> : m.display_name[0]?.toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate text-foreground">{m.display_name}</p>
